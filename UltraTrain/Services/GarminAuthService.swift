...
protocol GarminAuthServiceProtocol: Sendable {
...
final class GarminAuthService: GarminAuthServiceProtocol, @unchecked Sendable {
    private let logger = Logger(subsystem: "com.ultratrain", category: "garmin")
    private var accessToken: String?
    private var refreshToken: String?
    private var tokenExpiry: Date = .distantPast

    private var clientId: String { AppConfiguration.garminClientId }
    private var clientSecret: String { AppConfiguration.garminClientSecret }
    private let redirectURI = "ultratrain://garmin-callback"
    private let keychainKey = "garmin_tokens"
    ...
    func authenticate() async throws {
        ...
        logger.info("Garmin Connect authenticated successfully")
    }
    ...
    func disconnect() async {
        ...
        logger.info("Garmin Connect disconnected")
    }
    ...
    private func performOAuth() async throws -> String {
        var components = URLComponents(string: "https://connect.garmin.com/oauthConfirm")!
        components.queryItems = [
            URLQueryItem(name: "client_id", value: clientId),
            URLQueryItem(name: "response_type", value: "code"),
            URLQueryItem(name: "redirect_uri", value: redirectURI),
            URLQueryItem(name: "scope", value: "activity_export"),
        ]
        ...
        return try await withCheckedThrowingContinuation { continuation in
            let session = ASWebAuthenticationSession(
                ...
                guard let callbackURL,
                    ...
                    continuation.resume(throwing: GarminError.noAuthCode)
                    return
                }
                continuation.resume(returning: code)
            }
            session.presentationContextProvider = OAuthPresentationProvider.shared
    ...
    private func exchangeCodeForToken(code: String) async throws {
        var request = URLRequest(url: URL(string: "https://connectapi.garmin.com/oauth-service/oauth/token")!)
...
enum GarminError: Error, LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .notAuthenticated: "Not authenticated with Garmin Connect. Please connect your account."
        case .noAuthCode: "Failed to receive authorization code from Garmin."
        }
    }
}
