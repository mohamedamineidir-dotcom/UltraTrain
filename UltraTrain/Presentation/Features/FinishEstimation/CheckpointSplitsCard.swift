        ...
        return VStack(alignment: .leading, spacing: 2) {
            HStack(spacing: 0) {
                ...
                Text(formatElev(split.segmentElevationGainM))
                    .frame(width: 42, alignment: .trailing)
                Text(FinishEstimate.formatDuration(optimistic, raceDistanceKm: race.distanceKm))
                    .frame(width: 52, alignment: .trailing)
                Text(FinishEstimate.formatDuration(expected, raceDistanceKm: race.distanceKm))
                    .frame(width: 62, alignment: .trailing)
                    .fontWeight(.medium)
                Text(FinishEstimate.formatDuration(conservative, raceDistanceKm: race.distanceKm))
                    .frame(width: 52, alignment: .trailing)
            }
    ...
    private var finishRow: some View {
        HStack(spacing: 0) {
            ...
            Text(formatElev(race.elevationGainM))
                .frame(width: 42, alignment: .trailing)
            Text(FinishEstimate.formatDuration(estimate.optimisticTime, raceDistanceKm: race.distanceKm))
                .frame(width: 52, alignment: .trailing)
            Text(FinishEstimate.formatDuration(estimate.expectedTime, raceDistanceKm: race.distanceKm))
                .frame(width: 62, alignment: .trailing)
                .fontWeight(.bold)
            Text(FinishEstimate.formatDuration(estimate.conservativeTime, raceDistanceKm: race.distanceKm))
                .frame(width: 52, alignment: .trailing)
        }
        .font(.caption.monospacedDigit())
    }
